...
const DetailsHero = () => {
  const { tittle } = useParams() as { tittle: string };
  ...
  const blogArray: Blog[] = ARTICLES_CARD_LIST.filter(
    (obj) =>
      obj &&
      obj.title &&
      obj.title.toLowerCase().replace(/\s+/g, "-") === tittle
  );

  const newBlogs: Blog | null = blogArray.length > 0 ? blogArray[0] : null;
          ...
          <div className="container mt-[91px] max-w-[1140px] mx-auto">
            <div className="max-w-[481px]">
              ...
              <div className="gap-4 items-center flex">
                <CustomButton
                  title=" Productivity"
                  customClass="py-[6px] hover:opacity-100 border border-light-green text-nowrap xl:px-[31px] opacity-80 px-6 bg-light-black text-white shadow-none hover:text-light-green hover:bg-white rounded-full text-sm leading-[21px] xl:min-w-[154px]"
                />
                <CustomButton
